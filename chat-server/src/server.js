...
// TEST API
// app.get("/getAllRoom", async (_, res) => {
//   let useQuery = await db.query("SELECT * FROM message_room");
//   if (useQuery.rows.length > 0) {
//     return res.json(useQuery.rows);
//   }
...
app.get("/chat-rooms/:userId", async (req, res) => {
  ...
  console.log(userId)
  try {
    console.log("Fetching chat rooms for userId 123:", userId);
    ...
    console.error("Error fetching chat rooms server:", error);
...
app.get("/messages/:roomId", async (req, res) => {
  ...
  try {
    const messagesQuery = await db.query(
      `SELECT * FROM (
         SELECT id, sender_id, message_text, timestamp, is_read
         FROM messages
...
app.post("/auth/role", async (req, res) => {
  ...
  try {
    ...
    const driverResult = await db.query(
      `SELECT id FROM "driver" WHERE email = '${email}'`
    );
    ...
    return res
      .status(404)
      .json({ error: "Email not found in user or driver tables" });
  } catch (error) {
...
// POST /chat/room-id
app.post("/chat/room-id", async (req, res) => {
  const { userId, driverId } = req.body;

  try {
    // Kiểm tra xem room đã tồn tại chưa
    const existingRoom = await db.query(
      "SELECT id FROM message_room WHERE user_id = $1 AND driver_id = $2",
      [userId, driverId]
    );
    ...
    // Nếu chưa có, tạo mới room
    const newRoom = await db.query(
      "INSERT INTO message_room (user_id, driver_id) VALUES ($1, $2) RETURNING id",
      [userId, driverId]
    );

    return res.json({ roomId: newRoom.rows[0].id });
  } catch (error) {
    console.error("Lỗi khi tạo hoặc lấy room:", error);
