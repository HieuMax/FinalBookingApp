...
const GoogleTextInput = ({
  ...
  const fetchSuggestions = async (text: string, type: "start" | "end") => {
    ...
    const url = `https://api.locationiq.com/v1/autocomplete?key=${process.env.EXPO_PUBLIC_LOCATIONIQ_API_KEY}&q=${encodeURIComponent(text)}&limit=20&countrycodes=vn`;
    try {
  ...
  const debouncedSearch = useCallback(
    debounce(
      (text: string, type: "start" | "end") => fetchSuggestions(text, type),
      500,
    ),
    [],
  );
